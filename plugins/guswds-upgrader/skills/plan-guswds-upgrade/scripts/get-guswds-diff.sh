...
# Find the package.json that belongs to the Gesso USWDS (guswds) theme.
...
find_guswds_package_json() {
  # Walk up
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/package.json" ]] && grep -q '"name".*guswds' "$dir/package.json"; then
  ...
  found="$(find "$search_root" -name "package.json" \
    ...
    -exec grep -l '"name".*guswds' {} + 2>/dev/null | head -1)"
  if [[ -n "$found" ]]; then
    echo "$found"
    return 0
  fi
...
PACKAGE_JSON="$(find_guswds_package_json)" || {
  echo "Error: Could not find a Gesso USWDS (guswds) package.json in any parent directory or within the project" >&2
  exit 1
}
...
echo "Next Gesso USWDS release: $NEXT_VERSION"
echo ""
